...
import {
  ArrowLeft, Users, Swords, GraduationCap,
  Hash, Copy, CheckCircle, Brain, ChevronRight, AlertCircle, Loader2,
} from 'lucide-react';
...
const MODES: {
...
}[] = [
  {
    ...
    label: 'Collaborative',
    tag: 'Co-op',
    desc: 'Shared editor, two cursors. Agent coaches both players in real-time.',
    color: '#3b82f6',
  },
  {
    id: 'battle',
    ...
    label: 'Battle',
    tag: 'Competitive',
    desc: 'Independent editors. Race to solve. Agent compares both solutions.',
    color: '#f43f5e',
  },
  {
    id: 'mentor',
    ...
    label: 'Mentor',
    tag: 'Learning',
    desc: 'Mentor sees hints only. Learner codes. Agent coaches privately.',
    color: '#10b981',
  },
];
...
export default function MultiplayerLobbyPage() {
  ...
  const handleCreate = useCallback(async () => {
    ...
    setError('');
    try {
      const { roomCode: code } = await createRoom(selectedMode, sessionId, displayName);
      setRoomCode(code);
      // Short delay so user sees the code, then navigate to lobby
      setTimeout(() => router.push(`/multiplayer/${code}`), 1200);
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Failed to create room');
      setCreating(false);
    }
  }, [selectedMode, sessionId, displayName, router]);
  ...
  const handleJoin = useCallback(async () => {
    ...
    try {
      ...
      await joinRoom(code, sessionId, displayName);
      router.push(`/multiplayer/${code}`);
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Room not found or already started');
      setJoining(false);
    }
  }, [joinCode, sessionId, displayName, router]);
      ...
      <nav style={{
        ...
        {userName && (
          <div style={{ marginLeft: 'auto', fontSize: 13, color: '#475569' }}>
            Playing as <span style={{ color: '#e2e8f0', fontWeight: 600 }}>{displayName}</span>
          </div>
        )}
      </nav>
      ...
      <main style={{ maxWidth: 760, margin: '0 auto', padding: '56px 24px' }}>
        ...
        >
          <h1 style={{
            ...
            fontSize: 40, fontWeight: 900, letterSpacing: '-0.03em', marginBottom: 12,
          }}>
            Multiplayer Rooms
          </h1>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
                  ...
                  <span style={{
                    ...
                    letterSpacing: '0.07em', padding: '3px 9px', borderRadius: 99,
                  ...
                  {!roomCode ? (
                    <>
                      ...
                      <div style={{
                        ...
                        color: '#93c5fd', fontSize: 13, fontWeight: 600, marginBottom: 28,
                      }}>
                      ...
                      <button
                        ...
                        style={{
                          ...
                          background: creating ? 'rgba(59,130,246,0.4)' : 'linear-gradient(135deg, #2563eb, #7c3aed)',
                          color: 'white', fontSize: 15, fontWeight: 700, border: 'none',
                          cursor: creating ? 'not-allowed' : 'pointer',
                          boxShadow: creating ? 'none' : '0 0 32px rgba(59,130,246,0.3)',
                        }}
                      >
                        {creating
                          ? <><Loader2 size={18} style={{ animation: 'spin 0.7s linear infinite' }} /> Creating…</>
                          : <><Users size={18} /> Create Room <ChevronRight size={16} /></>
                        }
                      </button>
                    </>
                  ) : (
                    <>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16, marginBottom: 16 }}>
                        <div style={{
                          ...
                          <span style={{
                            ...
                            fontWeight: 900, color: '#93c5fd', letterSpacing: '0.25em',
                          }}>{roomCode}</span>
                        </div>
                        <button
                          onClick={handleCopy}
                      ...
                      <p style={{ fontSize: 12, color: '#475569' }}>Redirecting to lobby…</p>
                    </>
                  )}
                </motion.div>
              )}
                ...
                >
                  ...
                  <div style={{
                    ...
                    <input
                      ...
                      placeholder="DSAXYZ"
                      style={{
                        width: 200, padding: '16px 20px', borderRadius: 14,
                    ...
                    <button
                      ...
                      style={{
                        ...
                        background: (joinCode.length >= 4 && !joining)
                          ? 'linear-gradient(135deg, #2563eb, #7c3aed)'
                          : 'rgba(148,163,184,0.1)',
                        color: (joinCode.length >= 4 && !joining) ? 'white' : '#475569',
                        fontSize: 15, fontWeight: 700, border: 'none',
                        cursor: (joinCode.length >= 4 && !joining) ? 'pointer' : 'not-allowed',
                        display: 'flex', alignItems: 'center', gap: 8,
                      }}
                    >
                      {joining
                        ? <><Loader2 size={16} style={{ animation: 'spin 0.7s linear infinite' }} /> Joining…</>
                        : 'Join →'
                      }
                    </button>
                  </div>
                  <p style={{ color: '#334155', fontSize: 13 }}>
                    You must be logged in — both players need an account for cross-device play
                  </p>
                </motion.div>
              )}
            </AnimatePresence>
