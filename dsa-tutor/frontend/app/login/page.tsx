...
import {
  Brain, Mail, Lock, Eye, EyeOff, Zap, AlertCircle, ChevronRight, Sparkles,
} from 'lucide-react';
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (d < 130) {
            ...
            ctx.strokeStyle = `rgba(59,130,246,${0.12 * (1 - d / 130)})`; ctx.lineWidth = 1; ctx.stroke();
          }
        }
      }
      nodes.forEach((n) => {
        ctx.beginPath(); ctx.arc(n.x, n.y, n.r, 0, Math.PI * 2);
        ctx.fillStyle = 'rgba(99,160,255,0.5)'; ctx.fill();
        n.x += n.vx; n.y += n.vy;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      <div style={{
        ...
        border: `1px solid ${error ? 'rgba(244,63,94,0.5)' : focused ? 'rgba(59,130,246,0.6)' : 'rgba(148,163,184,0.12)'}`,
        borderRadius: 12, background: focused ? 'rgba(59,130,246,0.04)' : 'rgba(15,23,42,0.6)',
        boxShadow: focused ? '0 0 0 3px rgba(59,130,246,0.12)' : 'none',
        transition: 'all 0.2s',
      }}>
        <span style={{ position: 'absolute', left: 14, color: error ? '#f43f5e' : focused ? '#60a5fa' : '#475569', display: 'flex', transition: 'color 0.2s' }}>
          {icon}
        </span>
        <input
          id={id}
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        if (data.user) {
          const res = await fetch(`/api/auth/profile?auth_id=${data.user.id}`);
          if (res.ok) {
            const { sessionId, full_name } = await res.json();
            if (sessionId) {
  ...
  return (
    <div style={{
      ...
      {/* Ambient glow blobs */}
      <div aria-hidden style={{ position: 'fixed', top: '20%', left: '10%', width: 400, height: 400, borderRadius: '50%', background: 'radial-gradient(circle, rgba(59,130,246,0.08) 0%, transparent 70%)', pointerEvents: 'none', zIndex: 0 }} />
      <div aria-hidden style={{ position: 'fixed', bottom: '15%', right: '8%', width: 350, height: 350, borderRadius: '50%', background: 'radial-gradient(circle, rgba(139,92,246,0.07) 0%, transparent 70%)', pointerEvents: 'none', zIndex: 0 }} />
          ...
          <div style={{
            ...
            <AnimatePresence>
              {success && (
                ...
                >
                  <Zap size={16} /> Signed in! Redirecting…
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <form onSubmit={handleLogin} noValidate style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <a href="/forgot-password" style={{ fontSize: 12, color: '#60a5fa', textDecoration: 'none', fontWeight: 500 }}>
                  Forgot password?
                </a>
              </div>
              ...
              <motion.button
                ...
                style={{
                  ...
                  background: loading || success ? 'rgba(59,130,246,0.4)' : 'linear-gradient(135deg, #2563eb, #7c3aed)',
                  color: 'white', fontSize: 15, fontWeight: 700, letterSpacing: '-0.01em',
                  boxShadow: loading || success ? 'none' : '0 0 32px rgba(59,130,246,0.35)',
              ...
              >
                {loading ? (
                  ...
                  <>
                    <Brain size={17} /> Sign In <ChevronRight size={16} />
                  </>
                )}
              </motion.button>
            </form>
            ...
            <a href="/register" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '13px', borderRadius: 14, border: '1px solid rgba(148,163,184,0.15)', background: 'rgba(255,255,255,0.03)', color: '#94a3b8', fontSize: 14, fontWeight: 600, textDecoration: 'none', transition: 'all 0.2s' }}>
